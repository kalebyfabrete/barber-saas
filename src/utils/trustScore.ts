...
export function calculateTrustScore(
  ...
  let level: 'vip' | 'regular' | 'low'
  let requiresPrepayment: boolean
  let prepaymentPercentage: number
...
export function shouldBlockClient(
  ...
  totalBookings: number,
  score: number
): { blocked: boolean; reason?: string; blockDays?: number } {
...
export function getNoShowFee(
  noShowCount: number,
  servicePrice: number
): number {
  // Multa aumenta com o número de no-shows
  if (noShowCount === 0) return 0
  if (noShowCount === 1) return servicePrice * 0.5
  if (noShowCount === 2) return servicePrice * 0.75
