...
export function useServices(organizationId) {
    return useQuery({
...
export function useBarbersAvailability(organizationId, date) {
    return useQuery({
...
export function useAvailableSlots(_organizationId, barberId, serviceId, selectedDate) {
    return useQuery({
        ...
        queryFn: async () => {
            ...
            if (error)
                throw error;
            return generateTimeSlots(selectedDate, service.data.duration_minutes, bookings || []);
        },
    ...
    return useMutation({
        mutationFn: async (data) => {
            ...
            const { data: booking, error } = await supabase
                ...
                ends_at: new Date(new Date(data.scheduledAt).getTime() +
                    service.data.duration_minutes * 60000).toISOString(),
...
function generateTimeSlots(date, durationMinutes, bookedSlots) {
    const slots = [];
    const startHour = 9;
    const endHour = 18;
