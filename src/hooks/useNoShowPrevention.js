...
export function useTrustScore(clientId) {
    return useQuery({
        ...
        queryFn: async () => {
            ...
            if (!data)
                return null;
            const calculation = calculateTrustScore(data.total_bookings, data.completed_bookings, data.no_show_count, data.cancelled_count, data.trust_score);
            return {
                ...data,
                ...calculation,
            };
...
export function useUpdateClientStatus() {
    return useMutation({
        mutationFn: async ({ clientId, status, }) => {
            ...
            const newCalculation = calculateTrustScore(updates.total_bookings, updates.completed_bookings || client.completed_bookings, updates.no_show_count || client.no_show_count, updates.cancelled_count || client.cancelled_count);
            ...
            const blockCheck = shouldBlockClient(updates.no_show_count || client.no_show_count, updates.total_bookings, newCalculation.score);
...
export function useGetNoShowFee(clientId, servicePrice) {
    return useQuery({
...
export function useClientBlockStatus(clientId) {
    return useQuery({
...
//# sourceMappingURL=useNoShowPrevention.js.map